...
As these are already fast enough, it's not considered necessary to expand the template tree and include schedules or deadline in some non-root template.
...
Mostly copied from doom default config:
